...
#ifdef ROACH_PHY_HACK
/* National DP83865 */
static int dp83865_init(struct mii_phy *phy)
{
	int ctl;

	printk(KERN_WARNING "emac%d: not advertising 1000Mbit capability for DP83865\n",
		       ((struct ocp_enet_private *)phy->dev->priv)->def->index);

	ctl = phy_read(phy, MII_CTRL1000);
  ctl &= ~(ADVERTISE_1000FULL | ADVERTISE_1000HALF);
	phy_write(phy, MII_CTRL1000, ctl);

	return 0;
}

static int dp83865_setup_aneg(struct mii_phy *phy, u32 advertise)
{
	int ctl, adv;
...
static struct mii_phy_ops dp83865_phy_ops = {
	.init		= dp83865_init,
	.setup_aneg	= dp83865_setup_aneg,
	.setup_forced	= genmii_setup_forced,
	.poll_link	= genmii_poll_link,
	.read_link	= genmii_read_link
};

static struct mii_phy_def dp83865_phy_def = {
	.phy_id		= 0x20005c70,
	.phy_id_mask	= 0xfffffff0,
	.name		= "National DP83865 Ethernet",
	.ops		= &dp83865_phy_ops
};
#endif
