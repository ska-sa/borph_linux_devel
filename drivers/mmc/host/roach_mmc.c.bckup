...
static int __devinit rmmc_check(struct rmmc_host *host)
{
  ...
  if (rev_major < 1){
    printk(KERN_ERR "%s: cpld unprogrammed, mmc disabled.\n", "roach-mmc");
    return -ENODEV;
  }
  if (rev_major < RMMC_REV_SUPPORT){
    printk(KERN_ERR "%s: your current cpld firmware does not support mmc.\n", "roach-mmc");
    return -ENODEV;
  }
  host->irq = 1;
...
{
  if (!request_mem_region((resource_size_t)ROACH_CPLD_BASE,
                           ROACH_CPLD_LENGTH, "roach-cpld-mmc")) {
    printk(KERN_ERR "%s: memory range 0x%08x to 0x%08x is in use\n",
                          "roach-cpld",
                          (u32) ROACH_CPLD_BASE,
                          (u32)(ROACH_CPLD_BASE + ROACH_CPLD_LENGTH));
    return ENOMEM;
  }

  host->base = ioremap(ROACH_CPLD_BASE, ROACH_CPLD_LENGTH);
#ifdef RMMC_DEBUG
  printk(KERN_DEBUG "rmmc-wrtest: ioremap - req = %x, ret = %x\n", (u32)ROACH_CPLD_BASE, (unsigned int) host->base);
#endif
...
static int __devinit rmmc_init(struct device *dev)
{
  ...
  np = of_find_compatible_node(NULL, NULL, "kat,rmmc");
  if (np == NULL) {
#ifdef RMMC_DEBUG
    printk("rmmc: of find node failed\n");
#endif
    return -ENOENT;
  }
  ...
  //ret = request_irq(RMMC_IRQ_NUM, rmmc_irq, 0, DRIVER_NAME, (void*)(host));
  irq = irq_of_parse_and_map(np, 0);
#ifdef RMMC_DEBUG
  printk("rmmc: got irq from dtree, %d\n", irq);
#endif
  ret = request_irq(RMMC_IRQ_NUM, rmmc_irq, 0, DRIVER_NAME, (void*)(host));

  if (ret){
#ifdef RMMC_DEBUG
    printk("rmmc: request irq failed, %d\n", ret);
#endif
...
static int __devexit rmmc_remove(struct platform_device *dev)
{
  ...
  free_irq(RMMC_IRQ_NUM, (void*)(host));
...
static const struct of_device_id rmmc_of_match[] = {
	{ 
    .type       = "mmc",
    .compatible = "kat,rmmc"
  },
...
MODULE_DEVICE_TABLE(of, rmmc_of_match);
